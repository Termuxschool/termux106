...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6282347594640:+94 76 7285455\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '!'
blocked = []
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '[] Sucesso Boa Sorte',
				levelon: '[] *Ativado leveling*',
				leveloff: '[] *Desativado leveling*',
				levelnoton: '[] *Leveling nao ativado*',
        		...
        		const bisakah = ['lata','Tidak Bisa']
		        const kapankah = ['Mais dia','Mais domingo','Bulan Lagi','Tahun Lagi']
			const botNumber = client.user.jid
			const ownerNumber = ["6282347594640@s.whatsapp.net"] // replace this with your number
			...
			const NomerOwner = '6282347594640@s.whatsapp.net'
		...
		client.sendMessage(from, nsfwmenu(prefix, sender), text, {quoted: mek})
				...
				case 'demote':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, adicionar posi��o como administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
			...
			case 'simih':
				...
				case 'chord':
					if (args.length < 1) return reply('Onde esta o t�tulo da musica')
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Cad� a letra da m�sica ?')
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					tels = body.slice(7)
           ...
           break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Tchau kekick😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'bugreport':
				client.updatePresence(from, Presence.composing) 
				if (args.length < 1) return reply('Qual � o bug, ou queira falar comigo?')
					tek = body.slice(10)
					bug = {
					...
					client.sendMessage(from, 'Seu relat�rio foi enviado para o propriet�rio do BOT, relat�rio falso/main2 n�o ser� respondido.', text, {quoted: mek})
					break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    /*if (!isDaftar) return reply(mess.only.daftarB)*/
					if (!isQuotedSticker) return reply('❌ Respomda A Imagem Com O Comando ❌')
					reply(mess.wait)
                	...
                	client.updatePresence(from, Presence.composing) 
                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
					if (!isQuotedVideo) return reply('Responda o v�deo por favor')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
               ...
               if (!isGroup) return reply(mess.only.group)
                ...
                if (args[0] === '1') {
                    ...
                    reply('*Digite 1 Para Ativar O Recurso, 0 Para Desativar* \n *Exemplo: ${prefix}leveling 1*')
                }
            break
					case 'ping':
					...
					case 'lionlogo':
							...
							if (args.length < 1) return reply('Onde esta o texto?\nExemplo: ${prefix}lionlogo texto1|texto2')
							reply(mess.wait)
					...
					case 'jokerlogo':
							var gh = body.slice(11)
							var teks1 = gh.split("|")[0];
							if (args.length < 1) return reply('Onde esta o texto?\n Exemplo: ${prefix}jokerlogo seu texto')
							reply(mess.wait)
